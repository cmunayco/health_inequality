...
randomf3 <-plm(log(ir_tb) ~ log(hexp), data=df, index=c("country", "year"), model="random")
summary(randomf3)
...
#### STEP 5
randomf1 <-plm(log(ir_tb) ~ log(gdp) + log(hexp) + log(pnotified) + log(impsfac) + log(t_jail), data=df, index=c("country", "year"), model="random")
summary(randomf1)
